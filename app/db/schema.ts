import { mysqlTable, mysqlSchema, AnyMySqlColumn, index, foreignKey, primaryKey, varchar, timestamp, datetime, longtext, tinyint } from "drizzle-orm/mysql-core"
import { sql } from "drizzle-orm"

export const follow = mysqlTable("follow", {
	id: varchar("id", { length: 255 }).notNull(),
	follower: varchar("follower", { length: 255 }).notNull().references(() => user.id, { onDelete: "cascade" } ),
	followee: varchar("followee", { length: 255 }).notNull().references(() => user.id, { onDelete: "cascade" } ),
