  ...
  return (
    <div>
      ...
      <Form method='post'>
        ...
        <TextInput type='text' name='username' className='mb-10' value={formValues.username} onChange={(e) => setFormValues((prev) => ({ ...prev, username: e.target.value }))} />
        <label className='text-gray-500' htmlFor='username'>
          Bio
        </label>
        <TextInput name='bio' className='mb-10' value={formValues.bio} onChange={(e) => setFormValues((prev) => ({ ...prev, bio: e.target.value }))} />
        <div className='flex'>{isUpdating ? <Spinner /> : <Button className='text-white bg-blue-500 hover:bg-blue-600 py-1 px-4 rounded-lg mr-3 w-20 flex justify-center'>Save</Button>}</div>
      </Form>
    </div>
  );
}
